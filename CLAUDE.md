...
- The site is a committed **Nuxt 4.5 + @nuxt/content v3** scaffold using the `app/` directory, with `@nuxtjs/i18n` on `prefix_except_default`. The builder **is** the landing page (`app/pages/index.vue`); `/build` is a redirect document left for old links. `/parts/<item number>` is a prerendered part page per locale (764 routes, enumerated in `nuxt.config.ts` from `content/parts/`), while `/parts` itself is still the prose stub the content catch-all serves. `/kits/…` and chassis pages do not exist yet.
...
- `public/thumbs/` holds 695 generated 160×120 product thumbnails (382 parts, 305 kits, 8 chassis, 2.7 MB) plus 382 320×240 copies of the parts under `parts-detail/` (2.6 MB), which only the part page renders. All are written by `npm run catalog:thumbs` and committed. Rows with no photo draw an SVG slot icon from `app/utils/icons.ts` through `app/components/CatalogThumb.vue`.
