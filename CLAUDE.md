...
- `public/thumbs/` holds 695 generated 160×120 product thumbnails (382 parts, 305 kits, 8 chassis, 2.7 MB), written by `npm run catalog:thumbs` and committed. Rows with no photo draw an SVG slot icon from `app/utils/icons.ts` through `app/components/CatalogThumb.vue`.
