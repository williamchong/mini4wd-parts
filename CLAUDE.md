...
- The site is a committed **Nuxt 4.5 + @nuxt/content v3** scaffold using the `app/` directory, with `@nuxtjs/i18n` on `prefix_except_default`. The builder **is** the landing page (`app/pages/index.vue`); `/build` is a redirect document left for old links. The catalog browses as a two-level hierarchy per locale, all of it enumerated in `nuxt.config.ts` from `content/` rather than crawled: `/parts` (a 29-card category grid), `/parts/category/<slug>` (the rows, with a chassis filter kept in the `#hash`), `/parts/<item number>`, plus `/chassis` and `/chassis/<id>`. **URLs are flat and the hierarchy is declared** — `app/components/Breadcrumbs.vue` emits the trail and its `BreadcrumbList` JSON-LD, because GitHub Pages cannot 301 a URL that has already been shared (docs/PLAN.md §4.6). `@nuxtjs/sitemap` writes `/sitemap.xml` (848 URLs, 319 KB, `/build` and image entries excluded) and `public/robots.txt` points at it. `/kits/…` pages do not exist: a kit row on a chassis page opens the builder on that kit instead, through `encodeBuild`.
