...
**3D is primary and the first thing seen; the list is the accessibility, efficiency and backup path** (owner, 2026-09-09). Those are three distinct jobs and only one of them is a fallback:
...
- *Backup.* M1 models **one chassis of eight**, and any part without a generator draws a labelled proxy rather than a recognisable shape. A reader on VZ, or on a slot whose parts are all grey boxes, has nothing to click that means anything.

Only the third of those expires. So the list is not scaffolding to be removed when the meshes land — the first two jobs are permanent, and the question is which surface opens by default, not which one exists.
