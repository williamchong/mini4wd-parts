...
- **Localise `loadoutEntry.label`, and render the third origin (§4.8).** First of the remaining chunks: the label bug doubles when the kit door opens rather than staying constant, and scoping found it far smaller than it looked. Ships with the three-state origin badge, which cannot be seen until a kit is in play.
- `/build` kit entry point: kit picker searching by name or item number, overlaying `stockLoadout` on the chassis default. ~~Filtered to current kits~~ — **corrected 2026-09-09:** the corpus is 181 `current`, 124 `limited` and **zero** discontinued, so filtering to `current` would hide 124 kits people own. Show both and label the limited ones. Kit door primary, bare chassis the labelled fallback beside it ("can't find your kit, or starting from a bare chassis?"), which is also how the data reads: the 13 kits without a wiki loadout already fall back to the chassis default. Kits go into the prerendered payload alongside the parts — **not** fetched client-side: they add ~25–35 KB over the wire (the route is 185 KB raw but 24 KB brotli), where a browser-side `queryCollection` would pull 865 KB of SQLite WASM plus a 210 KB worker to save ~30 KB. Nothing on the site issues one today; do not be the first for this.
